...
<div class="container mt-4">
    <form class="row g-2 align-items-end mb-3" method="get" action="${ctx}/admin/violations">
        <div class="col-md-2">
        ...
        <div class="col-md-2">
            ...
            <input class="form-control" name="deviceId" value="${deviceId}">
        </div>
        <div class="col-md-3">
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <div class="fw-bold">违规记录</div>
                <a class="btn btn-outline-secondary btn-sm" href="${ctx}/admin/violations/stats">查看统计</a>
            </div>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <c:forEach items="${violationPage.list}" var="v">
                        <tr>
                            ...
                            <td>${v.status}</td>
                            <td><a class="btn btn-sm btn-outline-primary" href="${ctx}/admin/violations/${v.violationId}">详情</a></td>
                        </tr>
                    </c:forEach>
                ...
                <nav>
                    <ul class="pagination">
                        <li class="page-item ${violationPage.page <= 1 ? 'disabled' : ''}">
                            <a class="page-link" href="${ctx}/admin/violations?page=${violationPage.page-1}&size=${violationPage.size}&status=${status}&userId=${userId}&deviceId=${deviceId}">上一页</a>
                        </li>
                        <c:forEach begin="1" end="${totalPages}" var="p">
                            <li class="page-item ${violationPage.page == p ? 'active' : ''}">
                                <a class="page-link" href="${ctx}/admin/violations?page=${p}&size=${violationPage.size}&status=${status}&userId=${userId}&deviceId=${deviceId}">${p}</a>
                            </li>
                        </c:forEach>
                        <li class="page-item ${violationPage.page >= totalPages ? 'disabled' : ''}">
                            <a class="page-link" href="${ctx}/admin/violations?page=${violationPage.page+1}&size=${violationPage.size}&status=${status}&userId=${userId}&deviceId=${deviceId}">下一页</a>
                        </li>
                    </ul>
                </nav>
            </c:if>
